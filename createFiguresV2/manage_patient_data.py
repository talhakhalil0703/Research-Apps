...
def fill_trajectories_for_patient(single_patient, start_trajectories,end_trajectories, all_files):
    for index, start_trajectory in enumerate(start_trajectories):
        if single_patient.name == find_patient_number.search(start_trajectory):
            trajectory = []
...
def create_patient_array_with_trajectories(data_path, trajectories, all_files):
    ...
    for start_trajectory in start_trajectories:
        ...
        if patient_number is not old_patient:
    ...
    for trajectory in single_patient.trajectory_number:
        for mm_file in trajectory:
            for single_file in all_result_files:
                if mm_file.name is file_name.search(single_file)[0]:
                    temp_file = extract_single_file_data(single_patient, single_file, data_path)
                    if temp_file.r2 >= r2_tolerance:
                        mm_file.mm_files.append(temp_file.copy())
            x += 1

def append_brain_section_for_mm(mm_file, brain_section):
    temp_exponent, temp_offset, temp_r2, temp_error = None, None, None, None
    for segment in mm_file:
        ...
        brian_section.exponents.append(segment.exponent)
        brian_section.offset.append(segment.offset)
        brian_section.r2.append(segment.r2)
        brian_section.error.append(segment.error)

        for index, freq in enumerate(segment.peak_freq):
            brian_section.peak_freq.append(freq)
            brain_section.freq_area.append(segment.freq_area[index])

    brain_section.average_exponents.append(temp_exponent)
    brain_section.average_offset.append(temp_offset)
    brain_section.average_r2.append(temp_r2)
    brian_section.average_error.append(temp_error)
...
def extract_mm_from_middle(patient_array, dorsal, ventral, mm_to_extract, do_not_run_mm):
    for patient in patient_array:
        for trajectory in patient.trajectory_number:
            trajectory_length = len(trajectory) - 1 if len(trajectory) % 2 == 1 else trajectory_length = len(trajectory)
            ...
            for do_not_include in do_not_run_mm:
                if do_not_include is trajectory[ventral_index - mm_to_extract].name:
                    include_dorsal = False
                if do_not_include is trajectory[ventral_index - 1 + mm_to_extract].name:
